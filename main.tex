...
\begin{frame}[fragile]{Jagged cross-join has an array programming solution (Jaydeep)}
\small
\vspace{0.2 cm}
...
def cross(self, other):
    ...
    left[indexes] = self.offsets[parents[indexes]][:-1] + (
        (indexes - offsets[parents[indexes]]) //
            other.counts[parents[indexes]])
    right[indexes] = other.offsets[parents[indexes]][:-1] + \
        (indexes - offsets[parents[indexes]]) - \
            other.counts[parents[indexes]] * (
                (indexes - offsets[parents[indexes]]) //
                    other.counts[parents[indexes]])

    return Jagged.fromoffsets(offsets, Table(offsets[-1], left, right))
\end{minted}
\end{columns}
\end{frame}
